...
from agentops.sdk.decorators import workflow, operation, task
    ...
    for phy_item in toc_physical_index:
        for page_item in toc_page:
            if phy_item.get('title') == page_item.get('title'):
                physical_index_raw = phy_item.get('physical_index')
                # normalize to integer if possible (convert_physical_index_to_int handles several formats)
                try:
                    pi_int = None
                    if isinstance(physical_index_raw, int):
                        pi_int = physical_index_raw
                    else:
                        pi_int = convert_physical_index_to_int(physical_index_raw)
                except Exception:
                    pi_int = None

                if pi_int is not None and pi_int >= start_page_index:
                    pairs.append({
...
def generate_toc_continue(toc_content, part, model=None):
    ...
    # Model hit token limit — continue generating until complete
    max_attempts = 5
    for _ in range(max_attempts):
        ...
        cont_prompt = "Please continue the JSON structure, directly output the remaining part of the JSON array."
        ...
        if finish_reason == 'finished':
            return extract_json(response)
    raise Exception(f'generate_toc_continue: could not complete after retries (last finish reason: {finish_reason})')
...
def generate_toc_init(part, model=None):
    ...
    for _ in range(max_attempts):
        ...
        if finish_reason == 'finished':
            return extract_json(response)
    raise Exception(f'generate_toc_init: could not complete after retries (last finish reason: {finish_reason})')

@task(name="process_no_toc")
...
@task(name="process_toc_no_page_numbers")
...
@task(name="process_toc_with_page_numbers")
